...
```{r schools_size, fig.cap = "Number of schools per province (number on the left) and school size."}
...
Figure \@ref(fig:schools_size) shows that there are `r unname(unlist(school_by_community[1,"n"]))` Dutch speaken, `r unname(unlist(school_by_community[3,"n"]))` French speaking and `r unname(unlist(school_by_community[2,"n"]))` bilingual schools.
It is remarkable to see that there are no schools from the Limburg province and only one from Vlaams-Brabant.

```{r paticip_age_sex, fig.cap = "Year of birth (if known) and sex of the participating child."}
...
data_particip %>% 
  ...
  geom_bar() +
  geom_text(data = nb_child_by_province, aes(x = "2010", label = n,  y=38)) +
  facet_grid(rows = vars(province_name), cols = vars(biological_sex))
