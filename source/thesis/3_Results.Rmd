...
It is remarkable to see in Figure \@ref(fig:schools-size) and \@ref(fig:paticip-age-sex) that there are no schools from Limburg and only one from Vlaams-Brabant.
...
For the B@seball project's main research questions on the link between school environment and the children's microbiome, it is important to see that most children have spent quite some years in the same school already.
...
- Within 300m surrounding the school limit, but excluding the area of the 100m buffer.
...
```{r eda-school-greenery, fig.cap="Number of schools (left) and number of children (right) with a high or low naturalness and greenness around and within the school.", message=FALSE, warning=FALSE, fig.height = 4}
p1 <- schools %>%
...
### Attitude towards outdoor play in children
...
@beyer2015development prescribes that the ATOP scale for children has two sub-scales: ATOP_benefits and ATOP_fears (see Table \@ref(tab:atop-translation-kids) in the appendix). We apply multi-group Confirmatory Factor Analysis (CFA) and we hypothesize that the `r length(unique(atop_data$question_code))` items can be explained by these two common factors. 
As described in the methodology, we apply a grouping based on the language of the survey (French or Dutch).

Correlated error terms for two pairs of items were added to the model of @beyer2015development to improve the fit: (1) between the statements *calm_down* and *learn_new_things* and (2) between *calm_down* and *feeling_free*.
We hypothesize that calming down and feeling free are two similar emotions and, in contrast, learning new things typically requires you to be alert instead of calm.
Tables \@ref(tab:atop-loadings) and \@ref(tab:atop-loadings2)  in the appendix show the raw and standardized model estimates.
It can be seen that there is indeed a statistically significant positive correlation between *calm_down* and *feeling_free* for the French-speaking group. There is a significant negative correlation between *calm_down* and *learn_new_things* for the Dutch-speaking group.
...
Measurement invariance analysis shows that a model that assumes metric measurement invariance cannot be rejected using a Chi-Squared difference test ($\chi^2$ = `r round(anov_atop_metric[2,"Chisq diff"], 3)`, $\mathrm{df} =$ `r round(anov_atop_metric[2,"Df diff"])`, $p=$ `r round(anov_atop_metric[2,"Pr(>Chisq)"], 3)`) while a model that assumes strong measurement invariance is rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_atop_strong[2,"Chisq diff"], 3)`, $\mathrm{df} =$ `r round(anov_atop_strong[2,"Df diff"])`, $p=$ `r round(anov_atop_strong[2,"Pr(>Chisq)"], 3)`).
...
We therefore need to conclude that the internal consistency is barely acceptable. The composite reliability is better for ATOP_benefits than for ATOP_fears (perhaps due to the fact the one statement was left out of the original set of scale questions) and the French translation work slightly better than the Dutch translation according to McDonald's $\omega$. 
...
Among the `r nrow(children)` children that were selected for the study, `r length(unique(nc_data$child_id))` children filled in the NC questions. Table \@ref(tab:nc-descriptives) shows the descriptive statistics for NC scale items in all children. While the original article suggested to use a 7-point likert scale, a 5-point likert scale was used for brevity [@hunt2017monitor]. 

In preliminary tests of the survey with a few children, some questions were not clear enough. They were then clarified with some more details between brackets (see Table \@ref(tab:nc-translation) in the appendix)
...
We apply multi-group CFA and we hypothesize that the `r length(unique(nc_data$question_code))` items can be explained by one common factor (Nature Connectedness (NC)).
...
Table \@ref(tab:nc-loadings) and \@ref(tab:nc-loadings2) in the appendix show the raw and standardized model estimates. It can be seen that "I always find nature very beautiful" and "I always treat nature with respect" indeed are positively correlated in the French-speaking group.
...
Measurement invariance analysis shows that a model that assumes metric measurement invariance cannot be rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_nc_metric[2,"Chisq diff"], 3)`, $\mathrm{df} =$ `r round(anov_nc_metric[2,"Df diff"])`, $p=$ `r round(anov_nc_metric[2,"Pr(>Chisq)"], 3)`) while a model that assumes strong measurement invariance is rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_nc_strong[2,"Chisq diff"], 3)`, $\mathrm{df} =$ `r round(anov_nc_strong[2,"Df diff"])`, $p=$ `r round(anov_nc_strong[2,"Pr(>Chisq)"], 3)`).
We therefore select the model with metric measurement invariance as the preferred model and all fit measures indicate very good fit (according to @byrne1994structural, see Table \@ref(tab:nc-cfa-fit)).
...
### Risk engagement and protection survey in parents
...
The Risk Engagement and Protection Survey (REPS) was based on @olsen2018risk. This scale was also supplemented by two additional questions:
...
Figure \@ref(fig:reps-descriptives2) in the appendix shows that Dutch and French-speaking parents seem to answer slightly differently. Dutch-speaking parents answered more positive, on average, on the "risk engagement" statements (statement 1 to 6 in Figure \@ref(fig:reps-descriptives2)) and more negative on the "protection from injury" statements (statement 7 to 14 in Figure \@ref(fig:reps-descriptives2)). 
...
For REPS, it is hypothesized that there are two underlying factors: "Protection from injury" and "Engagement with risk" [@olsen2018risk]. We will test in Exploratory Factor Analysis (EFA) whether the two additional statements about playing in the rain and washing hands can improve the REPS scale.
...
We therefore test in two separate EFAs whether the two additional statements about playing in the rain and washing hands can improve the REPS scale.
...
Secondly, we use the mifa package to execute an EFA with multiple imputation in which case we can use the data of all parents (details in appendix \@ref(repsefa)).
Both of these analyses show that the statements on playing in the rain or washing hands do not fit well with the REPS subscales and are therefore ignored in the CFA that follows (see appendix \@ref(repsefa)). Additionally, the EFA's confirm that there are two factors in the data (one for protection from injury and one for risk engagement), as we expected.
...
Tables \@ref(tab:reps-loadings) to \@ref(tab:reps-loadings4) in the appendix show the raw and standardized coefficient estimates.
...
Measurement invariance analysis shows that a model that assumes metric measurement invariance can be rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_reps_metric[2,"Chisq diff"], 3)`, $\mathrm{df} =$ `r round(anov_reps_metric[2,"Df diff"])`, $p=$ `r round(anov_reps_metric[2,"Pr(>Chisq)"], 3)`) a model that assumes strong measurement invariance is also rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_reps_strong[2,"Chisq diff"], 3)`, $\mathrm{df} =$ `r round(anov_reps_strong[2,"Df diff"])`, $p=$ `r round(anov_reps_strong[2,"Pr(>Chisq)"], 3)`).
...
In an attempt to improve the fit of the model, EFA was tested with three or four factors (see appendix \@ref(reps-improve-fit)). Neither fit nor composite reliability improved when allowing for more than two factors.
```{r reps-cfa-fit}
...
Since the response rate was disappointing at first, some questions were left out in a shortened version of the questionnaire to motivate parents to fill it in (see also Table \@ref(tab:atop-translation) in the appendix).

Table \@ref(tab:atop-p-descriptives) shows the descriptive statistics for each of the questions. Figures \@ref(fig:opp1) to \@ref(fig:opp3) in the appendix show the same data visually for both languages separately.
...
Since there are no less than 22 questions asked about the indoor and outdoor activities of the children, we first use EFA for dimension reduction (see appendix \@ref(efa1)). We uncover nine underlying factors. 

Table \@ref(tab:externalval) shows the Pearson correlation between the factor scores of the nine newly discovered factors and each of the ATOP, NC, and REPS subscale factor scores. This allows us to check whether there is an association between the factor scores of ATOP, NC, and REPS on the one hand, and the indoor and outdoor activities of the children on the other.
The stars in Table \@ref(tab:externalval) indicate whether the correlation is significantly different from zero.
Most of the relations seem very intuitive, except for ATOP_fears.
For instance, natural play like building huts is associated with slightly higher ATOP_benefits, higher NC, higher REPS_risk_engagement, and lower REPS_protection_from_injury.
However, we would also expect it to be associated with lower ATOP_fears (as in @beyer2015development), however, the opposite is true; it is also related to slightly higher ATOP_fears.
It is interesting to see that higher REPS_protection_from_injury is associated with more screen time and more time spend on homework while ATOP_benefits is associated with a lower amount of screen time and NC is associated with less time for homework.
REPS_risk_engagement also shows positive correlations with indoor activities, semi-natural play, made-up games, and playing hide and seek.

```{r externalval-getfactorscores, message=FALSE, warning=FALSE, include=FALSE, echo = FALSE}
...
Lastly, the parents answered questions on the independent mobility of their child, inspired by @shaw2013children. They assess how much freedom they allow their children to travel around their own neighbourhood or city without adult supervision.
This survey on independent mobility was previously executed in England and Germany over the years, starting in 1971.
Researchers concluded that there has been a large reduction in independent mobility for primary school children over time.
...
Therefore, the number of responses is greatly reduced and we cannot be sure that the people who filled in the full survey represent an unbiased sample (Table \@ref(tab:atop-p-response-difference)). We therefore need to be careful when interpreting the results.

Table \@ref(tab:im-descriptives) shows the descriptive statistics for the eight questions on independent mobility and Figure \@ref(fig:im-nlvsfr) in the appendix shows the descriptive statistics for the Dutch-speaking and French-speaking parents separately.
As in the previous section, we first perform dimension reduction, using EFA.
Appendix \@ref(efa2) details the EFA for the independent mobility questions. We uncovered 4 latent factors in the EFA.


Table \@ref(tab:externalval-im) shows the Pearson correlation between the factor scores of the four newly discovered independent mobility factor scores and each of the ATOP, NC, and REPS subscale factor scores. This allows us to check whether there is an association between the factor scores of ATOP, NC, and REPS on the one hand, and the independent mobility of the children on the other.
The stars in Table \@ref(tab:externalval) indicate whether the correlation is significantly different from zero. 
...
Parents with high REPS_risk_engagement are more likely to allow their child to go out alone after dark, engage with traffic on their own, or explore their neighborhood alone or with friends.
Parents with high REPS_protection_from_injury tend to *not* allow their child to go out alone after dark or engage with traffic.
Neither of the REPS subscales had a statistically significant correlation with allowing a child to use public transport on their own.
...
```{r externalval-im-getscores, message=FALSE, warning=FALSE, include=FALSE, echo = FALSE, eval = TRUE}
im_data2 <- im_data  %>%
...
```{r externalval-im, message=FALSE, warning=FALSE, include=TRUE, echo = FALSE, eval = TRUE}
tbl2 <- im_data2 %>%
  dplyr::select(child_id, starts_with("f_")) %>%
  left_join(full_atop_scores %>%
              dplyr::select(child_id, starts_with("ATOP"))) %>%
...
We include the ATOP and NC scales with the following additional covariances between some of the statements as specified in section \@ref(scale-questions). Since we want to know how the three latent factors are related to each other, we include covariances between the three factors. Lastly, we add a regression for each latent factor with the following exogenous variables as covariates:

- The gender of the child (1 for male, 0 for female).
- The socio-economic status of the child as a continuous variable (see section \@ref(socio-economic-status)). We include the child-level SES, if available. Otherwise, we use the school-level SES as a proxy to avoid too much missingness.
- The school context / type of playground as a binary variable (low (coded as 0) versus high greenness (coded as 1), see section \@ref(school-environment)).
- The landscape type around the school as a binary variable (low (coded as 0) versus high naturalness (coded as 1), see section \@ref(school-environment)).

The B@seball project members also suggested to add air pollution (based on black carbon levels, nitrogen oxide levels, and particulate matter) and an interaction between the school and landscape environment. A multi-group SEM with a grouping by language (Dutch versus French) was also tested but yielded worse fit and reliability. We therefore will stick to a "simple" SEM for the whole data set. Since the multi-group analysis did not fit well, the language (Dutch versus French) was also tested as a possible covariate. Adding any of these covariates to the regression analyses, however, made the fit worse and did not yield any important additional insights.
...
<!-- The composite reliability ($\omega$) is `r round(unname(cr_omega_sem[1]), 3)` for ATOP_benefits, `r round(unname(cr_omega_sem[2]), 3)` for ATOP_fears, and `r round(unname(cr_omega_sem[3]), 3)` for NC. Cronbach Alpha is `r round(unname(cr_alpha_sem[1]), 3)` for ATOP_benefits, `r round(unname(cr_alpha_sem[2]), 3)` for ATOP_fears, and `r round(unname(cr_alpha_sem[3]), 3)` for NC. So, in line with the earlier reliability results in the CFA, the reliability of ATOP_fears is not satisfactory. We already concluded in section  \@ref(conclusion-on-the-scale-questions) that this latent factor may not be valid or reliable and that the statements should be reviewed. -->

Looking at the SEM results, some interesting conclusions can be drawn. Firstly, the only statistically significant correlation between the three latent factors is a positive correlation between ATOP_benefits and NC (`r round(sem_sum_std %>% filter(lhs =="ATOP_benefits", rhs== "NC") %>% dplyr::pull(est), 2)`, p-value `r round(sem_sum_std %>% filter(lhs =="ATOP_benefits", rhs== "NC") %>% dplyr::pull(pvalue), 2)`).
This indicates that children with a higher NC also tend to have a positive ATOP_benefits score.
...
It can be seen that boys tend to have lower ATOP_benefits but higher ATOP_fears.
...
This is not in line with the B@seball project's hypotheses.
Finally, higher SES positively influences all of the latent factors. This is largely in line with a recent study that also found a positive link between SES and environmental attitude in children in Germany [@bucht2024environmental].
