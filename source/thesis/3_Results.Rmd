...
It is remarkable to see that there are no schools from Limburg and only one from Vlaams-Brabant.
...
Covariances between the statements *calm_down* and *learn_new_things* and between *calm_down* and *feeling_free* were added to the model of @beyer2015development to improve the fit. Table \@ref(tab:atop-loadings) and \@ref(tab:atop-loadings2)  in the appendix show the raw and standardized model estimates.
...
Cronback alpha is `r round(cr_alpha_atop$ATOP_fears[1], 3)` and `r round(cr_alpha_atop$ATOP_fears[2], 3)` for ATOP_fears for the French and Dutch version respectively.
...
For comparison, our model attains a Cronbach alpha of `r round(cr_alpha_atop[1, 2], 3)` and `r round(cr_alpha_atop[1, 2], 3)` for the French and Dutch version respectively. This is slightly worse but still indicates sufficiently good reliability.
...
The language of the survey was chosen as the grouping for the CFA.
...
Firstly, we apply EFA using only the data of parents who filled in all questions (`r nrow(df)` parents).
...
Both of these analyses show that the statements on playing in the rain or washing hands do not fit well with the REPS factors and are therefore ignored in the CFA that follows (see section \@ref(repsefa) in the appendix).

Multigroup CFA was performed and, while both the fit and reliability was slightly better for a grouping based on the type of playground (green versus grey), we choose to present the results for a grouping based on the language for consistency purposes. 
...
A covariances between the following pairs of statements were added:
...
Table \@ref(tab:reps-loadings) to \@ref(tab:reps-loadings4) in the appendix show the raw and standardized model estimates.
...
A model with metric measurement measurement invariance was chosen which showed good fit.
However, the internal consistency ($\omega$ and $\alpha$) was barely acceptable.
The internal consistency of ATOP_fears is consistently worse than the internal consistency of ATOP_benefits.
The validity analysis (section \@ref(outdoor-play)) also showed that, while ATOP_benefits factor scores showed logical associations with outdoor play activities, ATOP_fears did not.
The researchers removed one ATOP_fears statement from the original scale since they deemed it to be inappropriate for younger children in the study ("I don't like playing outside in nature because there are people with drugs.").
...
Similar to ATOP, a model with metric measurement invariance is chosen which showed a good fit overall.
Composite reliability ($\omega$) was slightly better for Dutch-speaking  than French-speaking children but it was $>0.8$ for both groups.
...
Statements with low reliability definitely need to be revised in possible future iterations of these translations.
The validation analysis (section \@ref(validaty-analysis)) does show some logical relations with related variables so there still might be some merit in the factor scores which is encouraging.
...
A multi-group SEM with a grouping by language (Dutch versus French) was also tested but yielded worse fit and reliability. We therefore will stick to a "simple" SEM for the whole data set. In total, `r sem_sum$data$nobs` observations with complete data that were used in the SEM.
...
Looking at the SEM results, however, some interesting conclusions can be drawn. Firstly, the only statistically significant correlation between the three latent factors is a positive correlation between ATOP_benefits and NC (`r round(sem_sum_std %>% filter(lhs =="ATOP_benefits", rhs== "NC") %>% dplyr::pull(est), 2)`, p-value `r round(sem_sum_std %>% filter(lhs =="ATOP_benefits", rhs== "NC") %>% dplyr::pull(pvalue), 2)`). This indicates that children with a higher nature connectedness also tend to have a positive ATOP_benefits score. Secondly, we can assess each of the three regression models. The unstandardized  parameter estimates, standard deviations, and statistical significance are highlighted in Table \@ref(tab:sem-regression) (these can also be found in Table \@ref(tab:sem-results) in the appendix). It can be seen that boys tend to have higher NC and ATOP_benefits but higher ATOP-fears. A greener landscape around the school positively influences both NC and ATOP_benefits. The school context does not have much influence although a greener playground may impact ATOP_benefits slightly negatively. Finally, higher SES positively influences all of the latent factors.
