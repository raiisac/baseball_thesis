...
In total, `r nrow(likert_data_nc)` out of the `r nrow(children)` children filled in all of the NC questions and can therefore be analyzed further.
...
Secondly, we use the *mifa* package to execute an EFA with multiple imputation in which case we can use the data of `r nrow(mi$mids$data)` parents.
Both of these analyses show that the statements on playing in the rain or washing hands do not fit well with the REPS factors and are therefore ignored in the CFA that follows (see appendix \@ref(repsefa)). Additionally, the EFA with oblique rotation confirms there are two factors in the data (one for protection from injury and one for risk engagement), as we expected.
...
data.frame(
      ...
      REPS_risk_engagement = mapply(test_diff_cont, 2:5, "REPS_risk_engagement"),
      REPS_protection_from_injury = mapply(test_diff_cont, 2:5,
                                       "REPS_protection_from_injury")) %>%
  mutate(question_short = str_remove(question_short, "f_")) %>%
  kable(booktabs = TRUE, longtable = TRUE,
        caption = "Associations of ATOP, NC, and REPS factor scores with independent mobility latent factors (. p < 0.1, * p <.05, ** p <.01, *** p <.001).",
        col.names = c("", "ATOP benefits", "ATOP fears", "NC",
                      "REPS risk engagement", "REPS protection from injury"))  %>%
  kableExtra::kable_styling(latex_options = c("repeat_header"),
                            font_size = 9) %>%
...
The B@seball project members also suggested to add air pollution (based on black carbon levels, nitrogen oxide levels, and particulate matter) and an interaction between the school and landscape environment. Since the multi-group analysis did not fit well, the language (Dutch versus French) was also tested as a possible covariate. Adding any of these covariates to the regression analyses, however, made the fit worse and did not yield any important additional insights.
...
It can be seen that boys tend to have lower NC and ATOP_benefits but higher ATOP_fears.
