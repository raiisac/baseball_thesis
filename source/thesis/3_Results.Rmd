...
It is hypothesized that there are two underlying factors: "Protection from injury" and "Engagement with risk" [@@olsen2018risk].
...
reps_data %>%
  ...
  kable(booktabs = TRUE,
        ...
        col.names = c("Original statement", as.character(1:7), "Not answered")) %>%
  kableExtra::kable_styling(font_size = 9) %>%
  kableExtra::column_spec(1, width = "2in") %>%
  kableExtra::column_spec(2:9, width = "0.6in")
```
...
reps_data %>%
  ...
  ungroup() %>%
  print(n=31)
#wash_hands is never answered in the french community --> multiple imputation?
...
full_available <- reps_data %>%
  ...
  #          max = max(rest),
  #          mean = mean(rest)) %>%
  filter((community == "Dutch speaking" & n %% 16 == 0) | (community == "French speaking" & n %% 15 == 0)) %>%
...
p_fr <- likert_data_reps %>%
  ...
  gglikert(sort = "none", y_label_wrap = 20,
           labels_size = 2)
