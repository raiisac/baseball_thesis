...
While the initial goal was to get over 70 schools on board, only `r nrow(data_school)`.
The Covid 19 epidemic might also have had an influence on this disappointing response.
...
It is remarkable to see that there are no schools from the Limburg province and only one from Vlaams-Brabant.
Overall, the balance between boys and girls is good. Most children were the same age and born in 2011.
...
The mother usually filled in the parent's survey (Figure \@ref(fig:nb-years-in-school)).
...
likert_data_atop <- atop_data %>%
  ...
  left_join(children) %>%
  left_join(schools) 
model_atop <- "ATOP_benefits =~ think_clearly + good_for_health + calm_down + learn_new_things +   feeling_free + imagination + explore 
...
@beyer2015development prescribes that the ATOP scale for children had two sub-scales: ATOP-benefits and ATOP-fears (see Table \@ref(tab:atop-translation-kids) in the appendix). We apply multi-group confirmatory factor analysis (CFA) and we hypothesize that the `r length(unique(atop_data$question_code))` test scores can be explained by these two common factors. 
Different groupings were tested:

- A group for each school did not work well since there were very little students in some of the schools.
- A grouping per language (Dutch versus French) made a lot of sense since the translation may alter the interpretation of the statements slightly though we hope it did not.
- A grouping per school environment (urban or rural) and per type of playground (green or grey) was also tested.

The fit measures and composite reliability for the models with different groupings were compared. 
A grouping per language made the most sense and yielded a reasonable fit.
A correlation between the statements *calm_down* and *learn_new_things* and between *calm_down* and *feeling_free* were added to the model of @beyer2015development to improve the fit. 
...
Measurement invariance analysis showed that a model that assumes metric measurement invariance cannot be rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_atop_metric[2,"Chisq diff"], 3)`, $df =$ `r round(anov_atop_metric[2,"Df diff"])`, $p=$ `r round(anov_atop_metric[2,"Pr(>Chisq)"], 3)`) while a model that assumes strong measurement invariance is rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_atop_strong[2,"Chisq diff"], 3)`, $df =$ `r round(anov_atop_strong[2,"Df diff"])`, $p=$ `r round(anov_atop_strong[2,"Pr(>Chisq)"], 3)`).
We therefor select the model with metric measurement invariance as the preferred model and all fit measures, except the Chi-square test, indicate good fit (according to @byrne1994structural).
...
The composite reliability is `r round(cr_omega_atop$ATOP_fears[1], 3)` and `r round(cr_omega_atop$ATOP_fears[2], 3)` for ATOP_fears for the French and Dutch version respectively. We therefor need to conclude that the internal consistency is barely acceptable. It is better for ATOP_benefits than for ATOP_fears (perhaps due to the fact the one statement was left out of the original set of scale questions) and the French translation work slightly better than the Dutch translation. 
...
likert_data_nc <- nc_data %>%
  ...
  left_join(children) %>%
  left_join(schools) 
model_nc <- "NC =~ beauty + respect + happy + spending_time + enjoy + part_of
  beauty ~~ respect"
...
In total, `r nrow(likert_data_nc)` out of the `r nrow(children)` children filled in all of the Nature Connectedness questions and can therefor be analyzed further.
We apply multi-group confirmatory factor analysis (CFA) and we hypothesize that the `r length(unique(nc_data$question_code))` test scores can be explained by one common factor; Nature Connectedness (NC). 
Different groupings were tested:
...
The fit measures and composite reliability for the models with different groupings were compared. 
A grouping per language made the most sense and yielded a good fit.
...
Measurement invariance analysis showed that a model that assumes metric measurement invariance cannot be rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_nc_metric[2,"Chisq diff"], 3)`, $df =$ `r round(anov_nc_metric[2,"Df diff"])`, $p=$ `r round(anov_nc_metric[2,"Pr(>Chisq)"], 3)`) while a model that assumes strong measurement invariance is rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_nc_strong[2,"Chisq diff"], 3)`, $df =$ `r round(anov_nc_strong[2,"Df diff"])`, $p=$ `r round(anov_nc_strong[2,"Pr(>Chisq)"], 3)`).
We therefor select the model with metric measurement invariance as the preferred model and all fit measures indicate good fit (according to @byrne1994structural, see Table \@ref(tab:nc-cfa-fit)).

The composite reliability ($\omega$) for this model is `r round(cr_omega_nc[2, 2], 3)` and `r round(cr_omega_nc[1, 2], 3)` for the French and Dutch version respectively. For the original English scale, a chronbach alpha of 0.899 was reported in children [@hunt2017monitor]. For comparison, our model attains a Cronbach alpha of `r round(cr_alpha_nc[2, 2], 3)` and `r round(cr_alpha_nc[1, 2], 3)` for the French and Dutch version respectively. This is slightly worse but still indicates sufficiently good reliability.
...
Since there are a lot of statements (14 from REPS and 2 additional ones), these questions were removed from the shortened questionnaire to encourage overall response rate for the most important questions for the B@seball project's goals. Therefor, there are only `r nb_parents_reps` parents of `r nb_children_reps` children who filled in all REPS statements (for some children, more than one parent answered the survey).  All statements were answered on a 7-point likert scale. Some questions were coded from "Strongly disagree" to "Strongly agree" while others were coded "Never" to "Always".
...
Due to a mistake, the question on washing hands ("Should your child wash his/her hands after outdoor play?) was not included in the french survey (last column and row in Table \@ref(tab:reps-descriptives)). We therefor test in two seperate exploratory factor analysis whether the two additional statements about playing in the rain and washing hands can improve the REPS scale. Firstly, we apply EFA using only the data of parents who filled in all questions. We therefor also use the *mifa* package to execute an exploratory factor analysis with multiple imputation. 
...
Due to a mistake, the question on washing hands ("Should your child wash his/her hands after outdoor play?) was not included in the french survey (last column and row in Table \@ref(tab:reps-descriptives)). We therefor test in two separate exploratory factor analysis whether the two additional statements about playing in the rain and washing hands can improve the REPS scale. Firstly, we apply EFA using only the data of parents who filled in all questions (`r nrow(df)` parents). Secondly, we use the *mifa* package to execute an exploratory factor analysis with multiple imputation in which case we can use the data of `r nrow(mi$mids$data)` parents. Both of these analyses show that the statements on playing in the rain or washing hands do not fit well with the REPS factors and are therefor ignored in the CFA that follows (see \@ref(repsefa) in the appendix).

Multigroup CFA was performed using the same groupings as before. While both the fit and reliability was slightly better for a grouping based on the type of playground (green versus grey), we choose to present the results for a grouping based on the language for consistency purposes. 
...
model_reps <- "protection_from_injury =~ prevention_importance + concerned_injury + concerned_hazards + avoid_risk + importance_supervision + chance_of_injury + limit_dangerous_activities + encourage_safe_activities
...
The fit measures and composite reliability for the models with different groupings were compared.
A correlation between the following pairs of statements were added:

- prevention_importance and chance_of_injury
- concerned_injury and concerned_hazards
- concerned_hazards and avoid_risk
- prevention_importance and avoid_risk
...
Measurement invariance analysis showed that a model that assumes metric measurement invariance can be rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_reps_metric[2,"Chisq diff"], 3)`, $df =$ `r round(anov_reps_metric[2,"Df diff"])`, $p=$ `r round(anov_reps_metric[2,"Pr(>Chisq)"], 3)`) a model that assumes strong measurement invariance is also rejected using a Chi-Squared Difference Test ($\chi^2$ = `r round(anov_reps_strong[2,"Chisq diff"], 3)`, $df =$ `r round(anov_reps_strong[2,"Df diff"])`, $p=$ `r round(anov_reps_strong[2,"Pr(>Chisq)"], 3)`).
We therefor select the model with configural measurement invariance as the preferred model. However, all fit measures indicate poor fit (according to @byrne1994structural).
...
The composite reliability is `r round(cr_omega_reps$risk_engagement[2], 3)` and `r round(cr_omega_reps$risk_engagement[1], 3)` for "ATOP_fears"risk engagement" for the French and Dutch version respectively.
We therefor need to conclude that the internal consistency is not acceptable, especially for the French translation of the "protection from injury" factor.
Cronback $\alpha$ values indicate a reasonable fit for the Dutch translation (`r round(cr_alpha_reps$protection_from_injury[1], 3)` for protection from injury and `r round(cr_alpha_reps$risk_engagement[1], 3)` for risk engagement).
...
In this section, we look at additional information, obtained through the surveys, and check whether these variable correlate to the factor scores of the ATOP, NC, and REPS factors.
...
The last two rows in Table \@ref(tab:atop-p-response-difference) show that, while there is no statistically significant difference in the scores to ATOP_fears, children of parents who filled in the short questionnaire scores significantly higher on ATOP_benefits than those of parents who filled in the long questionnaire.

This means that there may be some selection bias in who filled in the full questionnaire and we will have to be carefull when interpreting the results.
...
Table \@ref(tab:externalval) shows whether there is an association between the factor scores and the indoor and outdoor activities of the children. For questions with a binary answer (the first 14 questions), the mean factor score for those who said yes/no is shown, the start indicate whether there is a statistically significant difference. For questions with a numeric answer, the correlation is shown and the start indicate whether it is significantly different from zero.
Most of the relations are intuitive, except for the ATOP_fears  associations. For instance, building huts is associated with higher ATOP_benefits, higher nature connectedness, higher REPS_risk_engagement and lower REPS_protection_from_injury. However, we would expect it to be associated with lower ATOP_fears but the opposite is true. It is also interesting to see that higher ATOP_benefits is associated with a lower amount of screen time while higher REPS_protection_from_injury is associated with a higher amount of screen time.
...
Lastly, the parents some questions on the independent mobility of their child, inspired by @shaw2013children. 
